# This class implements image creation based on GPT.
...
do_uefiapp[depends] += " \
                         ...
                         ${INITRD_IMAGE}:do_populate_sysroot \
...
# must be consistent regardless of the image format.
...
# The type used is associated to Windows recovery partitions, which makes it safe in
# this context of linux-only images.
PARTITION_TYPE_EFI_BACKUP = "2700"
# The EFI backup is implemented in sw, to avoid depending on BIOS features that might
#  be broken (EFI BIOSes have a tendency to not fulyl implement the EFI specs).
DSK_IMAGE_LAYOUT ??= ' \
{ \
